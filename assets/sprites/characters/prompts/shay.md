...
The real Player 1 for this Chore Quest instance. Drop `shay.png` (the source photo) into Nano Banana alongside this prompt.

## Prompt

Using the attached photo (`shay.png`) as the reference subject, create a 128×128 pixel art character sprite in classic 16-bit SNES / arcade-era style. This is "Shay" — Player 1 of Chore Quest, the real-life player rendered as a game-ready chibi hero in the style of Chrono Trigger, Secret of Mana, or Street Fighter II character portraits.

Translate from the photo — Shay must be recognizable as themselves:
...
- **Outfit**: replicate the outfit visible in the photo, redrawn blocky using only the palette. If the outfit is ambiguous or unclear, default to arcade-hero energy — a t-shirt + pants combo that pops against black, something a main-character would wear. Add subtle folds/shading via 2-color dithering, never true gradients.
...
- **Vibe**: confident smirk or small grin. Player 1 energy — this is the hero, not a neutral mannequin.
...
- [ ] Recognizable as the person in `shay.png`
...
`chore-quest/assets/sprites/characters/shay.png`
